...
export default function page() {
    ...
    useEffect(() => {
        ...
        const likedIds = userData.likedBlogs || [];
        const filtered = blogs.filter((blog) => likedIds.includes(blog._id));
        setLiked(filtered);
        setIsFiltering(false);
    }, [userData, blogs]);
    ...
    return (
        liked?.length > 0 ?
            ...
            <div className="container pt-4">
                <h2 className="4xl:text-[26px] 2xl:text-[24px] xl:text-[22px] md:text-[20px] text-[18px] font-semibold mt-2">No Liked Blogs</h2>
            </div>
    )
}